{% macro snowflake__get_external_build_plan(source_node) %}
    ...
    {%- set create_schema_forbidden = external.create_schema_forbidden -%}
    ...
    {% if source_node.external.get('snowpipe', none) is not none %}
        ...
        {% if create_or_replace %}
            {% set build_plan = build_plan + (
                ([dbt_external_tables.create_external_schema(source_node)] if not create_schema_forbidden else [])
                + [
